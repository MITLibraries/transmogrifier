...
.PHONY: help install venv update test coveralls lint lint-fix security check-arch dist-dev publish-dev docker-clean transform dist-stage publish-stage

##############################################
# Python Environment and Dependency commands
##############################################

install: .venv # Install Python dependencies and create virtual environment if not exists
	uv sync --dev
	$(MAKE) .git/hooks/pre-commit .git/hooks/pre-push
...
transform: # Show CLI usage via uv script entrypoint (command requires arguments)
	uv run transform --help
...
# Emergency-only custom stage targets are preserved, but aligned to buildx and
# architecture-aware tagging required by the spec.
...
docker-clean: # Clean up Docker detritus
	@ARCH_TAG=$$(cat .arch_tag 2>/dev/null || true); \
