...
class Datacite:
    ...
    def __next__(self) -> TimdexRecord:
        ...
        record = self.create_from_datacite_xml(
            self.source_symbol, self.source_base_url, self.source_name, xml
        )
        return record

    @classmethod
    def create_from_datacite_xml(
        cls, source_symbol: str, source_link_url: str, source_name: str, xml: Tag
    ) -> TimdexRecord:
        """
        Args:
            source_symbol: The symbol for the source repository that is prepended
            to the timdex_record_id.
            source_link_url: A direct link to the source metadata record.
            source_name: The name of the source repository.
            xml: A BeautifulSoup Tag generated by parsing Datacite XML data.
        """
        ...
        kwargs = {
            ...
            "timdex_record_id": f"{source_symbol}:{source_record_id.replace('/', '-')}",
    ...
    def generate_name_identifier_url(cls, name_identifier):
        ...
        Args:
            name_identifier: An BeautifulSoup Tag parsed from XML data containing
            a name identifier.
        """
